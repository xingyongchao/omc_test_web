...
export function getRankTable(yySetting, data, isInPanel, panelType, skinConfig) {
      ...
      tableCols.push(
        {
          title: <div style={skinStyle2}>{ele.caption}</div>,
          dataIndex: ele.valueField,
          render: (text, row, index) => renderLenthBar(text, row, index, ele, colorList[index], maxMeasureArr, isInPanel, panelType, bShowValueAfterBarCol)
        });
  ...
  data.forEach((eleData, dataIndex) => {
    ...
    tableCols.forEach((eleCols) => {
      ...
      if (fieldName.indexOf("_BarValue") > 0) {
        ...
        tmpData[fieldName] = <div style={skinStyle} className={getClassNamePrifx(panelType, "rankTable") + "_Col_Value"}>{eleData[fieldName]}</div>;
      }
    });

    if (bAddIndexCol) {
      tmpData[indexCol.key] = <div style={skinStyle} className={getClassNamePrifx(panelType, "rankTable") + "_Index_" + (dataIndex > 2 ? 4 : (dataIndex + 1))}>{dataIndex + 1}</div>;
    }
  ...
  return <div className={getClassNamePrifx(panelType, "rankTable") + (isInPanel ? "_Panel" : "_Report")}>
    <div
      className={getClassNamePrifx(panelType, "rankTable") + "_Title"}
      style={titleStyle}
    >
...
const getClassNamePrifx = (panelType, className) => {
  if (panelType == 2) {
    return className + "2";
  }
  else {
...
const renderLenthBar = (text, row, index, ele, barColor, maxMeasureArr, isInPanel, panelType, bShowValueAfterBarCol) => {
  // fieldname: ele.valueField, fieldvalue: 0, bShowValueAfterBar
  ...
  if (isInPanel) {//如果是大屏显示
    if (panelType == 1) {
      ...
      style = { height: '10px', width: (valuePer + "%"), backgroundColor: barColor, float: "left", borderTopLeftRadius: "0", borderBottomLeftRadius: "0", borderTopRightRadius: (valuePer > 10 ? "4px" : "0"), borderBottomRightRadius: (valuePer > 10 ? "4px" : "0") };
    }
  }
  else {
  ...
  return <div>
    <div style={style}>&nbsp;</div>
    <div><span className="ranknumber"> {(bShowValueAfterBar && (bShowValueAfterBarCol == false)) ? value : null}</span></div>
  </div>;
