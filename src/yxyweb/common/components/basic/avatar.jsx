  ...
  baseControl() {
    ...
    return (
      <Row className={`face-img ${className}`}>
        ...
        <span className="avatar-n">{tooltip || '仅支持JPG、PNG格式，文件小于2M'}</span>
        {/*<span className="avatar-n">{tooltip || '仅支持JPG、PNG格式，文件小于2M（建议使用镂空白色图像，最佳尺寸90*60px）'}</span>*/}
      </Row>
    );
  }
  getControl() {
    const { readOnly, bIsNull, caption } = this.state;
    const title = !readOnly && bIsNull === false && caption ? <label><Icon type='star' />{caption}</label> : <label>{caption}</label>;
    let control = (caption ? <Label titleClassName='logo-face-label' control={this.baseControl()} title={title} /> : this.baseControl());
    return control;
  }
  render() {
