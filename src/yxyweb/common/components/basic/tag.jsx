...
export default class TagControl extends React.Component {
  ...
  render() {
    ...
    let tagSuffix = null;
    let { popWidth, popLeft } = this.state;
    let extraProps = {};
    extraProps = {
    ...
    let _style = { height: '28px', width: '100%' }
    let _content = <div className='tag-refer-icon-container'>
      {addTag}
    </div>
    if (this.state.referType === 'Tree')
      tagSuffix = <Popover
        ...
        onVisibleChange={this.onVisibleChange}>{_content}</Popover>
    else if (this.state.referType === 'Table' && this.state.displaymode === 'popover')
      tagSuffix = <Popover
        {...extraProps}
        content={<ListTag viewModel={this.referViewModel} multiple={this.state.multiple} closePop={this.close} />}
        onClick={this.onPopClick}
    ...
    return (
      !bShowCaption ?
        ...
        <Row className={`tag-refer tag-refer-01 ${this.state.err}`} style={style}>
          ...
          <div className={className} ref={node => this.input = node} role="combobox" aria-autocomplete="list" aria-haspopup="true" tabindex="0" aria-expanded="false">
            {TagList}
            {tagSuffix}
          </div>
