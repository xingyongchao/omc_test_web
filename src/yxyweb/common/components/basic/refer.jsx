...
export default class ReferControl extends React.Component {
  ...
  handleBodyClick = (e) => {
    ...
    if (e.target === this.refs.inputSearch.refs.input || e.target.classList.contains('anticon-canzhao') || e.target.classList.contains('anticon-close-circle') || cb.dom(e.target).parents('.has-related').length) return;
    document.body.removeEventListener('click', this.handleBodyClick);
