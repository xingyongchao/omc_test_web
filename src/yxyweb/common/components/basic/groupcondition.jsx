...
class GroupCondition extends React.Component {
  ...
  getConditionCardRender() {
    ...
    let bEdit = editCondition.bEdit;
    let content;
    if (groupConditionState[this.getBillNum()].bShowCard == true) {
      let card;
      if (editCondition.isCrossTable)
  ...
  getGroupCardRender() {
    ...
    let eChart = groupConditionState[this.getBillNum()].eChart;
    let isCanShowInMobile = this.canShowInMobile();
    let chartTags = this.getChartTags();
    return <div>
      ...
      <Row className="crosstype">
        ...
        <div className="eChart-global">
          ...
          <span className="eChart_Setting"><Button disabled={(eChart.displayType == 2 || eChart.displayType == 3) ? false : true} onClick={() => this.showEChartSetting(true, "")}>增加图形报表</Button></span >
          {eChart.displayType > 1 ? <span className="Charts-columns">图表列数</span> : ""}
          {eChart.displayType > 1 ? <span className="Charts-columns-input">
            <Input
              placeholder="请输入"
              value={eChart.eChartConfig.subChartColNum}
              onChange={(e) => this.eChartSetting_SetSubChartColNum(e.target.value)}
            />
          </span> : ""}
          {eChart.displayType > 1 ? <span className="Charts-columns-input-hint">请输入<span>1-3</span>的整数</span> : ""}
         
        </div>
      </Row>
      {eChart.displayType > 1 ?
        <Row className="crosstype">
          <span className="crosstypeName crosstypeline"><i className="anticon anticon-star"></i>展示</span>
          {chartTags}
        </Row> : ""
      }
      <Row className="crosstype">
        ...
        <span className="eChart-ApplyAtMobile" style={{ cursor: "pointer" }} ><Checkbox disabled={!isCanShowInMobile} checked={editCondition.isMobile} onChange={(e) => this.editCondition_SetValue("isMobile", e.target.checked)} >移动端展现</Checkbox></span>
      </Row>
      <Row colCount={12}>
  ...
  getChartTags() {
    ...
    let bShowEChartSetting_Add = groupConditionState[this.getBillNum()].bShowEChartSetting_Add;
    let tags = [];
    if (subChartConfigArr) {
      subChartConfigArr.forEach(function (ele, index) {
        let bAddAndLastOne = (index == subChartConfigArr.length - 1 && bShowEChartSetting && bShowEChartSetting_Add);
        if (bAddAndLastOne == false) {
          ...
          let name = _.get(ele, textPath) || "图形";
          tags.push(<div className="eChart-SubChartTag" >
            <span className="eChart-SubChartTag-Name" onClick={() => self.showEChartSetting(true, ele.chartKey)}>
              {name}
            </span>
            <span className="eChart-SubChartTag-Icon" >
              <Icon
                onClick={() => self.removeConfig(ele.chartKey)}
                type={"guanbi1"}
              />
            </span>
          </div>);
    ...
    return <div className="eChart-global2">
      {tags}
    </div>;
  }
  ...
  canShowInMobile() {
    const { groupConditionState } = this.props;
    let isCanShowInMobile = true;
    let subChartConfigArr = _.get(groupConditionState[this.getBillNum()], 'eChart.eChartConfig.subChartConfigArr');
    if (subChartConfigArr) {
      if (_.findIndex(subChartConfigArr, function (o) { return o.yySetting.type == "ranktable" }) >= 0) {
  ...
  eChartSetting_SetSubChartColNum = (fieldValue) => {
    const { groupConditionState, groupConditionRedux } = this.props;
    if (fieldValue != "" && (isNaN(fieldValue) == true || Number(fieldValue) < 1 || Number(fieldValue) > 3)) {
      ...
      groupConditionRedux.eChartSetting_SetSubChartColNum(this.getBillNum(), fieldValue);
    }
  }
  ...
  removeConfig = (chartKey) => {
    // bShow 是否显示  true false
    // chartKey 为空则为新增，不为空则为编辑
    const { groupConditionState, groupConditionRedux } = this.props;
    // this.state.bCancelSetting = false;
    groupConditionRedux.eChartSetting_RemoveConfig(this.getBillNum(), chartKey);
  }

  showEChartSetting = (bShow, chartKey) => {
    // bShow 是否显示  true false
    // chartKey 为空则为新增，不为空则为编辑
    const { groupConditionState, groupConditionRedux } = this.props;
    // this.state.bCancelSetting = false;
    if (bShow) {
      if (!!chartKey) {
        let config = eChartCommon.subConfig_Get(groupConditionState[this.getBillNum()].eChart, chartKey);
        this.chart_Backup = config;
        this.chartKey_Backup = chartKey;
      }
      else {
        this.chart_Backup = undefined;
        this.chartKey_Backup = "";
      }
      groupConditionRedux.eChartSetting_Show(this.getBillNum(), bShow, chartKey);
    }
    else {
      groupConditionRedux.eChartSetting_CancelChartConfig(this.getBillNum(), this.chart_Backup, this.chartKey_Backup, !this.chartKey_Backup);
    }
  ...
  eChartSetting_getRender = () => {
    ...
    if (groupConditionState[this.getBillNum()].bShowEChartSetting == true) {
      content = <Modal
      ...
      >
        <div className="eChartSetting_Control">
          <EChartDesign
            ...
            chartKey={groupConditionState[this.getBillNum()].showEChartKey}
