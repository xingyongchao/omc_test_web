...
import TreeTableEditCell from '../grid/TreeTableEditCell';
...
export default class TreeTable extends Component {
  ...
  setListenerState(data) {
    ...
    this.setState({ columns, rowKey: data.keyField, readOnly: data.readOnly })
  }
  setDataSource(dataSource) {
    let { rowKey, columns } = this.state;
  ...
  getKeyMap = (data, rowKey) => {
    ...
    data.map(item => {
      ...
      if (this.state.expandAllRow) contrast[key].isExpand = true;
      ...
      if (item.children)
        contrast[key].children = this.getKeyMap(item.children, rowKey, {}, status);
    });
    return contrast;
  }
  /*设置展开行*/
  ...
  getColumn = (col) => {
    const { bIsNull, cShowCaption, fixedRight, iAlign, cItemName, iColWidth, bCanModify } = col;
    let headerCell = (<Cell><div className='billing-head-name'>{cShowCaption}</div></Cell>);
    if (bCanModify && !bIsNull && this.state.readOnly === false)
      headerCell = (<Cell><div className='billing-head-name'><div class="red-star">*</div>{cShowCaption}</div></Cell>);
    let width, align, fixed = false;
    ...
    if (iAlign === 1)
      ...
      align = 'right';
    if (fixedRight) {
      return (<Column key={cItemName}
        allowCellsRecycling columnKey={cItemName}
  ...
  setCell = (rowProperty, col) => {
    ...
    return (
      <Cell width={rowProperty.width} height={this.rowHeight} className="billing-table-cell">
        <div title={titile} className="cell-text" onClick={e => this.onCellClick(e, col, index)}>
          {text}
        </div>
      </Cell>
    );
  ...
  compareCell = (row, col, index, bExpand) => {
    ...
    const { cItemName, bCanModify, bJointQuery } = col;
    let text = cb.utils.isEmpty(row[cItemName]) ? "" : row[cItemName];
    // if (this.state.readOnly === false) {
    if (bCanModify && this.state.readOnly === false) {
      let bEditCell = false;
      if (bExpand) {
        bEditCell = this.cellStatus[index.key] ? this.cellStatus[index.key][cItemName] : false;
      } else {
        let key = this.state.dataSource[index][rowKey];
        bEditCell = this.cellStatus[key] ? this.cellStatus[key][cItemName] : false;
      }
      if (bEditCell) {
        return <TreeTableEditCell {...this.state} {...this.props} bExpand={bExpand} keyMap={this.keyMap}
          column={col} rowIndex={index} onCellBlur={this.onCellBlur} row={row}
        />
      }
    }
    // this.editCellModel
    ...
    return bJointQuery ? <a className="joinQueryCell" onClick={e => this.handleJointQuery(e, index, cItemName)}>{text}</a> : text;
  }
  onCellBlur = (value, bExpand, rowIndex, cItemName, updateData) => {
    let node, key;
    if (bExpand) {
      node = this.keyMap[rowIndex.key].row;
      key = rowIndex.key;
    } else {
      const { dataSource, rowKey } = this.state;
    ...
    if (updateData) {
      if (!cb.utils.isEmpty(value) && typeof value == 'object') {
        for (var a in value) {
          node[a] = value[a];
        }
      } else {
      ...
      this.props.model.updateNode(node);
    }
    this.cellStatus[key][cItemName] = false;
    this.setState({ rowKey: this.state.rowKey });
  }
  onCellClick = (e, col, index, isExpand) => {
    // if (this.state.readOnly) return;
    if (!col.bCanModify || this.state.readOnly) return;
    this.props.model.promiseExecute('cellClick', { col, index, isExpand }, () => {
      let { rowKey, dataSource } = this.state;
      let key;
      if (isExpand) {
        key = index;
      } else {
      ...
      if (this.cellStatus[key])
        this.cellStatus[key][col.cItemName] = true;
      this.setState({ rowKey })
    });
  }
  ...
  loopChildIndeterminate = (data, cItemName) => {
    ...
    for (var key in children) {
      ...
      let child = children[key].children;
      let cInde;
      if (child) cInde = this.loopChildIndeterminate(this.keyMap[children[key].key], cItemName);
      if (childRow[cItemName] || cInde)
        checkedLen += 1;
    }
      ...
      if (index.showAction) {
        actionList.map(function (action) {
          if (this.actionState[key] && this.actionState[key][action.cItemName].visible) {
            let temp_action = this.getPopControl(action, index);
  ...
  getPopControl = (action, index) => {
    ...
    if (action.popoverKey) {
      ...
      if (typeof index == 'object')
        ...
        rowData = this.state.dataSource[index];
      if (ComName)
        temp_action = (
          <ComName model={action.model} data={rowData}>
            {temp_action}
          </ComName>
        );
    }
  ...
  getExpandCol = (row, parentKey) => {
    ...
    columns.map(col => {
      ...
      controls.push(
        <div className={expandColClass} onClick={e => this.onCellClick(e, col, parentKey, true)} id={parentKey} style={style}>{text}</div>
      );
    });
