...
import { Checkbox, Pagination, Popover, Select, Button } from 'antd';
import Row from './row';
import Col from './col';
...
export default class FixedDataTableControl extends React.Component {
  constructor(props) {
    ...
    this.lookUpRowIndexes = []; this.lookUpRow = -1; this.lookUpKey = '';
  }
  ...
  rowClassNameGetter = (index) => {
    ...
    let lookUpRowIndexes = this.lookUpRowIndexes;
    if (lookUpRowIndexes.length > 0) {
      let className = '';
      for (let i = 0; i < lookUpRowIndexes.length; i++) {
  ...
  switchCell = () => {
    ...
    let nowRowIndex = 0;
    let selectRowIndexes = this.props.model.getSelectedRowIndexes();
    if (!selectRowIndexes || selectRowIndexes.length > 1) return;
    nowRowIndex = selectRowIndexes[0];
    let data = { rowIndex: nowRowIndex, cellName: '' };
  ...
  setListenerState = (params) => {
    if (params.columnMode === 'local' && this.props.columns) {
      ...
      for (var attr in this.props.columns) {
        const modelColumn = params.columns[attr];
        if (!modelColumn) continue;
        const viewColumn = this.props.columns[attr];
    ...
    this.colsWidth = this.computeColsWidth(params.columns);
  ...
  setColumns = (columndata) => {
    ...
    if (this.state.columnMode === 'local' && this.props.columns) {
      const columns = {};
      for (var attr in this.props.columns) {
        const modelColumn = columndata[attr];
        if (!modelColumn) continue;
        const viewColumn = this.props.columns[attr];
        modelColumn.index = viewColumn.index;
        modelColumn.enterDirection = viewColumn.enterDirection;
    ...
    let tmp_ColIndexs = {}, index = 0, hasSpecialCol = false, specialCols = [];
    if (this.props.widthMode == 'percent') {
      let tableWidth = this.props.width || 1100;
    ...
    this.hasSpecialCol = hasSpecialCol;
    this.specialCols = specialCols;
    this.colsWidth = this.computeColsWidth(columndata);
    this.ColIndexs = tmp_ColIndexs;
    this.groupHeaderHeight = 0;
    this._InitCellStatus(columndata, this.dataList);
    ...
    if (this.props.tableTyep == 'rptTable') {
      ...
      this.GridProps.showAggregates = showAggregates;
    }
    this.setState({ columnset });

  }
  computeColsWidth = (columns) => {
    let colsWidth = 0;
    for (var key in columns) {
      let iColWidth = parseFloat(columns[key].iColWidth || 150);
      colsWidth += iColWidth;
    }
    return colsWidth;
  }
  ...
  setColumn = (attr, col) => {
    ...
    if (attr === 'CheckBox') {
    ...
    } else if (controlType == 'vouchermoney') {/*凭证 金额*/
      ...
      headerCell = (
        <Cell className={headerClassName}>
          <GridFilterModel fixedtable={fixedtable} multiSort={this.GridProps.multiSort} tableClass={this.props.tableClass} readOnly={this.readOnly} Column={col} name={name} model={this.props.model} width={width} attr={attr} data={this.dataList}
            onSortChange={this.onSortChange} sortColumn={this.sortColumn} lookUpRow={this.lookUpRow}
            moveLookUpRow={this.moveLookUpRow} lookUpKey={this.lookUpKey}
          />
        </Cell>
      )
    }
  ...
  moveLookUpRow = (lookUpRowIndexes, lookUpRow, key) => {
    if (lookUpRow > -1) this.scrollRow = lookUpRow;
    this.lookUpRowIndexes = lookUpRowIndexes;
  ...
  setCellEnter = (data) => {
    ...
    let enterDirection = column[data.cellName] ? column[data.cellName].enterDirection : null;
    if (enterDirection == null) return;
    let i = 1, columnKey, rowIndex;
    /*1向上 2向下 3向左 4向右*/
    ...
    if (!columnKey) return;
    this.CellStatus[data.rowIndex][data.cellName] = false;
    this._onCellClick(null, rowIndex, columnKey);
  }
  ...
  _setFooter = (cellProps) => {
    let column = this.columns;
    let { showAggregates, showSubtotal, showCheckBox, showRowNo } = this.GridProps;
    let footerTitle, footerCellStyle = { "padding": "8px 10px", "lineHeight": "14px" };
    if (showAggregates == true) {
      ...
      if (column[cellProps.columnKey].bNeedSum) {
        ...
        totalSum = this._getDecimal(this.columns[cellProps.columnKey], totalSum);
        return (
          <div className="fixedDataTable-footer-title">
            <div className='public_fixedDataTableCell_cellContent subtotal'>
              <Cell style={{ "height": this.props.footerHeigh || 38 }}>
                <div style={footerCellStyle} className="textCol" >
                  {sum}
                </div>
              </Cell>
            </div>
            <div className='public_fixedDataTableCell_cellContent aggregates'>
              <Cell style={{ "height": this.props.footerHeigh || 38 }}>
                <div style={footerCellStyle} className="textCol" >
                  {totalSum}
                </div>
      ...
      footerTitle = (
        <div className="fixedDataTable-footer-title">
          <div className='public_fixedDataTableCell_cellContent aggregates'> 合计</div>
        </div>
      );
      // if (showCheckBox && this.props.tableTyep != 'rptTable') {
      if (showCheckBox) {
    ...
    if (isPage && pagination.total !== 0) {
      ...
      return (
        <div className='pagination-new'>
          <Pagination showQuickJumper showSizeChanger pageSizeOptions={['10', '20', '30', '50', '80', '100', '1000', '2000', '3000', '65536']} showTotal={total => `共${total}条`} size="small" total={pagination.total} current={pagination.current} pageSize={pagination.pageSize} onShowSizeChange={this.onShowSizeChange} onChange={this._PaginChange} />
          ...
          bShowPop ?
            <Row style={{ minHeight: "25px" }} onMouseEnter={(e) => this.onMouseEnter(e, index)} onMouseLeave={(e) => this.onMouseLeave(e, index)}>
              <div className='pull-left' title={element.cShowCaption}>
                <Checkbox checked={showIt} onChange={(e) => this.onChecked(e, element, index)}>{element.cShowCaption || element.cCaption}</Checkbox>
              </div>
  ...
  getActionControl = () => {
    ...
    if (actionList && actionList.length > 0) {
      ...
      let lookUpRowIndexes = this.lookUpRowIndexes;
      if (lookUpRowIndexes.length > 0) {
        lookUpRowIndexes.forEach(index => {
  ...
  rowHeightGetter = (index) => {
    if (this.hasSpecialCol) {
      ...
      let maxRows = 0;
      this.specialCols && this.specialCols.map(speCol => {
        let cStyle = this.columns[speCol].cStyle;
        try {
          cStyle = JSON.parse(cStyle);
        } catch (e) {
        ...
        let specialDataSource = cStyle.dataSource;
        if (row[specialDataSource] && row[specialDataSource].length > maxRows)
          maxRows = row[specialDataSource].length
      });
      if (maxRows <= 1) return this.rowheight;
      return 30 * maxRows;
    } else {
