...
import UpLoadFace from 'yxyweb/common/components/account-center/UploadFace';
import * as systemSettingactions from 'yxyweb/common/redux/systemSetting';
...
class CompanyInfoControl extends Component {
                    ...
                    <Row>
                        <Col span={24}>
                            <div className="viewSetting viewCell width-percent-100">
                                <Row>
                                    <Col className='label-control logo-face-label' span={4}><label>企业LOGO</label></Col>
                                    <Col className='input-control' span={16}>
                                        <Row className="face-img companyInfo-logo-img">
                                            <UpLoadFace 
                                                ...
                                                imageChange={(url)=>this.logoChange(url, 'logo')}
                                            />
                                            <span>仅支持JPG、JPEG、BMP、PNG格式，文件小于1M（建议最佳尺寸144*60px）</span>
                                        </Row>
                                    </Col>
                                </Row>
                            </div>
    ...
    logoChange(url, type){
        let { systemSettingactions } = this.props;
        let obj = {};
        obj[type] = url
    ...
    saveData(data) {
        let saveLogo = this.state.dataSource.logo, saveBizLogo = this.state.dataSource.bizLogo;
        let { systemSettingactions, systemSetting } = this.props;
        var proxy = cb.rest.DynamicProxy.create({
            save: {
                url: '/tenant/save.do',
