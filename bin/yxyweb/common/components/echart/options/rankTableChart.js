...
function getRankTable(yySetting, data, isInPanel, panelType, skinConfig) {
    ...
    if (ele.bShowValueNotBar) {
      ...
      tableCols.push({
        ...
        render: function render(text, row, index) {
          return renderLenthBar(text, row, index, ele, colorList[index], maxMeasureArr, isInPanel, panelType, bShowValueAfterBarCol);
        }
      });
  ...
  data.forEach(function (eleData, dataIndex) {
    ...
    tableCols.forEach(function (eleCols) {
      ...
      if (fieldName.indexOf("_BarValue") > 0) {
        ...
        tmpData[fieldName] = _react2.default.createElement(
          'div',
          { style: skinStyle, className: getClassNamePrifx(panelType, "rankTable") + "_Col_Value" },
          eleData[fieldName]
        );
      }
    });
    ...
    if (bAddIndexCol) {
      tmpData[indexCol.key] = _react2.default.createElement(
        'div',
        { style: skinStyle, className: getClassNamePrifx(panelType, "rankTable") + "_Index_" + (dataIndex > 2 ? 4 : dataIndex + 1) },
        dataIndex + 1
      );
    }
  ...
  return _react2.default.createElement(
    'div',
    { className: getClassNamePrifx(panelType, "rankTable") + (isInPanel ? "_Panel" : "_Report") },
...
var getClassNamePrifx = function getClassNamePrifx(panelType, className) {
  if (panelType == 2) {
    return className + "2";
  } else {
    return className;
...
var renderLenthBar = function renderLenthBar(text, row, index, ele, barColor, maxMeasureArr, isInPanel, panelType, bShowValueAfterBarCol) {
  // fieldname: ele.valueField, fieldvalue: 0, bShowValueAfterBar
