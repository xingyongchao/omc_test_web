...
exports.components = exports.panelDefaultValue = exports.panelType = exports.chartDisplayType = exports.displayStyle = exports.eChartLabel = undefined;
...
var eChartLabel = exports.eChartLabel = {
  SplitChar: "/",
...
var displayStyle = exports.displayStyle = [1, //报表的展现方式：单表   
2, //报表的展现方式：单图     
3 //报表的展现方式：图+表  
];

var chartDisplayType = exports.chartDisplayType = {
  rpt: "rpt", //在最初的PC报表中展现
...
var panelDefaultValue = exports.panelDefaultValue = {
  borderColor: "#DBE0E5",
  backgroundColor: "",
  margin: "7px",
  allMargin: "18px",
...
function LogChartInfo(logTag, logStr, errNumber) {
  var tmpStr = "";
  if (errNumber == 999) console.log("图形报表日志 " + tmpStr + " 异常信息 " + logTag + (logStr === "" ? "" : " = " + logStr));else if (errNumber >= 901) console.log("图形报表日志 " + tmpStr + logTag + (logStr === "" ? "" : " = " + logStr) + " err=" + errNumber); //
}
  ...
  var config = {
    chart1: //饼形图
    {
      ...
      yySetting: {
        ...
        dataField: {
          dimensionX: [{ codeField: "area_code", nameField: "area_name", bLabel: true, iLabelOrder: 1 }, { codeField: "store_code", nameField: "store_name", bLabel: true, iLabelOrder: 2 }],
          measure: [{ valueField: "fRetailMoney" }]
        },
        radius: {
          radiusInner: 40, //内外半径
          radiusOuter: 65 //内外半径
    ...
    chart2: //柱形图，不存在次维度，多度量
    {
      ...
      yySetting: {
        ...
        dataField: {
          ...
          { codeField: "area_code", nameField: "area_name", bLabel: false, iLabelOrder: 1 }, { codeField: "store_code", nameField: "store_name", bLabel: true, iLabelOrder: 2 }],
          ...
          { valueField: "fMoney", caption: "金额", stack: "总额" }, { valueField: "fNetMoney", caption: "销售净额", stack: "累计" }, { valueField: "fRetailMoney", caption: "零售金额", stack: "累计" }]
        },
        xAxis: {
          axisLabel: {
    ...
    chart3: //折线图
    {
      ...
      yySetting: {
        ...
        dataField: {
          ...
          measure: [//度量 比如客户的零售金额 如果有次维度，则只能设一个度量，没有次维度可以设多度量
          { valueField: "fMoney", caption: "金额", smooth: true, stack: "总额" }, { valueField: "fNetMoney", caption: "销售净额", smooth: true, stack: "总额" }, { valueField: "fRetailMoney", caption: "零售金额", smooth: false, stack: "总额" }]
        },
        xAxis: {
          axisLabel: {
    ...
    chart4: //气泡图
    {
      ...
      yySetting: {
        ...
        dataField: {
          dimensionX: [//主维度的分组字段，是否显示由bLabel决定，比如根据门店+商品分组，但X轴只显示商品名字
          { codeField: "store_code", nameField: "store_code", bLabel: false, iLabelOrder: 1 }, { codeField: "store_name", nameField: "store_name", bLabel: true, iLabelOrder: 2 }],
          measure: [//度量 比如客户的零售金额 如果有次维度，则只能设一个度量，没有次维度可以设多度量
    ...
    chart5: //柱折图
    {
      ...
      yySetting: {
        ...
        dataField: {
          ...
          measure: [//度量 比如客户的零售金额 如果有次维度，则只能设一个度量，没有次维度可以设多度量
          { valueField: "fMoney", caption: "金额", stack: "总额", smooth: false, yAxisIndex: 0 }, { valueField: "fNetMoney", caption: "销售净额", stack: "累计", smooth: false, yAxisIndex: 1 }, { valueField: "fRetailMoney", caption: "零售金额", stack: "累计", smooth: false, yAxisIndex: 1 }]
        },
        xAxis: {
          axisLabel: {
            needWrap: false, //显示文字是否需要折行
    ...
    chart6: //排名表
    {
      ...
      yySetting: {
        ...
        dataField: {
          dimensionX: [{ codeField: "store_code", nameField: "store_code", caption: "门店编码", bLabel: false, iLabelOrder: 1 }, { codeField: "store_name", nameField: "store_name", caption: "门店名称", bLabel: true, iLabelOrder: 2 }],
          measure: [
          ...
          { valueField: "fMoney", caption: "金额", bShowValueNotBar: false, iOrder: 3, bShowValueAfterBar: true }, { valueField: "fNetMoney", caption: "销售净额", bShowValueNotBar: true, iOrder: 4, bShowValueAfterBar: false }, { valueField: "fRetailMoney", caption: "零售金额", bShowValueNotBar: true, iOrder: 5, bShowValueAfterBar: false }]
        }
      },
      eChartSetting: {}
    }
  };
  if (eChartType == "") return config;else {
    //取模版设置取默认模板
    var chart = void 0;
    if (eChartType == "pie") {
      chart = config.chart1;
    ...
    chart.createTime = (0, _formatDate.Format)(new Date(), 'yyyy-MM-dd hh:mm:ss');
    return chart;
  }
}

//主维度的显示名字和次维度的名字可能会重复，处理名字重复的问题
function UnionDimensionName(yySetting, data) {
  var dimensionX = yySetting.dataField.dimensionX.sort(function (a, b) {
    return a.iLabelOrder > b.iLabelOrder;
  });
  var dimensionSub = yySetting.dataField.dimensionSub;
  ...
  data.forEach(function (itemData) {
    xCode = "";
    xName = "";
    dimensionX.forEach(function (itemX) {
      ...
      if (itemX.bLabel == true) if (itemData.hasOwnProperty(itemX.nameField)) {
    ...
    if (dimensionSub && dimensionSub.length > 0) {
      var subItem = dimensionSub[0];
      var subCode = itemData[subItem.codeField];
      var subName = itemData[subItem.nameField];
  ...
  data.forEach(function (itemData) {
    try {
      if (_.isEmpty(itemData[eChartLabel.unionedXCode].toString())) itemData[eChartLabel.unionedXCode] = "空数据"; //主维度空值数据
      if (_.isEmpty(itemData[eChartLabel.unionedXName].toString())) itemData[eChartLabel.unionedXName] = "空数据"; //主维度空值数据
      if (dimensionSub && dimensionSub.length > 0) {
        if (_.isEmpty(itemData[eChartLabel.unionedSubCode].toString())) itemData[eChartLabel.unionedSubCode] = "辅维度空值数据";
        if (_.isEmpty(itemData[eChartLabel.unionedSubName].toString())) itemData[eChartLabel.unionedSubName] = "辅维度空值数据";
      }
    } catch (e) {
...
// }
//大屏看板
function getPanelLayOutTemplate1() {
  var temps = [];
  var temp1 = {
    "id": -1,
    "icon": "laobanfangan",
...
function reCalcTemplate2Height2AndRowsHeight(editPanel) {
  ...
  for (var i = 0; i < rows.length; i++) {
    var ele = rows[i];
    if (i != rows.length - 1) {
      var ratio = Math.floor(ele.height2 / height2 * 10000);
      ratio = ratio / 100;
      fRatio = ratio + fRatio;
      ele.height = ratio + "%";
    } else {
...
function reCalcRowsHeight2AndInnerRowsHeight(rowEle) {

  var maxNum = 0;
  rowEle.cols.forEach(function (colEle) {
    if (colEle.widgetType == "rows") {
      if (maxNum < colEle.widgetValue.length) maxNum = colEle.widgetValue.length;
    }
  });
  ...
  rowEle.cols.forEach(function (colEle) {
    if (colEle.widgetType == "rows") {
      ...
      for (var i = 0; i < colEle.widgetValue.length; i++) {
        var ele = colEle.widgetValue[i];
        if (i < maxNum - 1) {
          var ratio = Math.floor(1 / maxNum * 10000);
          ratio = ratio / 100;
          fRatio = ratio + fRatio;
          ele.height = ratio + "%";
        } else {
...
//桌面看板
function getPanelLayOutTemplate2() {
  var temps = [];
  var temp1 = {
    "id": -1,
    "icon": "icon-test",
...
function getPanelLayOutTemplateRow2() {
  var row = {
    rowKey: getNewRowKey(), // "001",
    height: "0%",
    backgroundColor: '',
...
//移动看板
function getPanelLayOutTemplate3() {
  var temps = [];
...
function getBackStyleArr(key) {
  ...
  if (key) return _.find(arr, function (o) {
    return o.backStyleKey == key;
  });else return arr;
}

function getBackColorArr(key) {
  // 1 主背景 // 2 标题背景 // 3 图形背景 // 4 汇总背景F
  ...
  if (key) return _.find(arr, function (o) {
    return o.backColorKey == key;
  });else return arr;
}

function getSkinArr(skinKey) {
  // 1 主背景 // 2 标题背景 // 3 图形背景 // 4 汇总背景F
  var arr = [];
  var arr1 = {
...
function restoreEditPanel(pageLayout, items, editType, reportViewId) {
  var editPanel = JSON.parse(pageLayout);
  if (editType == "edit") {
    //如果复制模板，会导致pageLayout里面的id和外面的ID不一致，在此修改一下
    if (reportViewId && editPanel.id != reportViewId) {
      editPanel.id = reportViewId;
    }
  } else if (editType == "query") {} else {
    editPanel.id = -1;
  }
...
function restoreEditPanelRows(rows, items) {
  ...
  rows.forEach(function (rowEle) {
    rowEle.cols.forEach(function (colEle) {
      ...
      if (colEle.widgetType == "sum" || colEle.widgetType == "chart") {
        if (colEle.widgetType == "sum") config = colEle.sumConfig;else config = colEle.panelChartConfig;

        item = _.find(items, function (o) {
          return o.itemKey == config.chartKey;
        });
...
function subConfig_Get(eChart, chartKey, bAdd) {

  var tmpItem = void 0;
  var subChartConfigArr = _.get(eChart, "eChartConfig.subChartConfigArr");
  var bHasArr = subChartConfigArr && subChartConfigArr.length > 0;
  if (bHasArr) {
    subChartConfigArr.forEach(function (item) {
      if (item.chartKey == chartKey) tmpItem = item;
    });
  }
...
function subConfigArray_Operate(eChart, chartKey, newConfig, operate) {
  var subChartConfigArr = _.get(eChart, "eChartConfig.subChartConfigArr");
  if (operate == "add") {
    ...
    if (subChartConfigArr && subChartConfigArr.length > 0) {
      subChartConfigArr.forEach(function (item, index) {
        if (item.chartKey == chartKey) tmpIndex = index;
      });
    }
    if (tmpIndex >= 0) {
...
function getColHeight(chartDisplayType, panelType, subChartColNum, width) {
  var height = "auto";
  if (chartDisplayType == "rpt") {

    if (subChartColNum == 1) height = 350;else if (subChartColNum == 2) height = 280;else if (subChartColNum == 3) height = 210;else if (subChartColNum == 4) height = 175;else if (subChartColNum == 5) height = 140;
  } else if (chartDisplayType == "mobile") {
    if (width == 0) {
      height = "auto";
    } else {
      if (subChartColNum == 1) height = width * 0.4;else if (subChartColNum == 2) height = width * 0.35;else if (subChartColNum == 3) height = width * 0.3;else if (subChartColNum == 4) height = width * 0.25;else if (subChartColNum == 5) height = width * 0.2;
      height = height * 1.5;
    }
